...
- **TRO-444** (demo video/social post) — script re-grounded on `6b60377b` and merged (PR #304, `cf9b4e4b`, 2026-08-16 ~16:45Z); Troy began the live pre-stage the same hour (his `pnpm dev` = API `:3001` / web `:5174`; `/tmp/ship-demo.env` written for him; P4 listener → `:8788` because `:8787` is held by a `Ship-wt-tro_444` process). X thread + 8 cards ready in `docs/submission/SOCIAL-THREAD-W6.md` / `social-assets/w6/thread/` (real captures). His dev DB was under-migrated (049) — migrated to 052 2026-08-16 ~17:10Z. Only the recording + posting remain his.
