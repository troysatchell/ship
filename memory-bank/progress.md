...
- **X thread built (same session, ~17:00–17:40Z):** `docs/submission/SOCIAL-THREAD-W6.md` — 8 posts (all ≤ 278 X-weighted chars) + 8 cards in `docs/submission/social-assets/w6/thread/` (`cards.html` → `render.mjs`, Soft-Structuralism treatment, deliberately unlike the W4-W5 black/Geist set). Captures are REAL and from Troy's own local stack: `ship login` with a Playwright-driven browser approve (real `/oauth-device-verify` frame), `ship webhooks tail` + `ship docs create` (created 16:58:57.983Z → ✓ verified 16:58:58.290Z = 307 ms), portal Replay ("Replay succeeded", attempt 7 with `Replayed from`), Audit page filtered to the CLI app. Two real breakers found on the way, both fixed for his recording: (1) his dev DB `ship_standup` was under-migrated (stopped at 049; `GET /api/v1/webhooks/deliveries` 500'd on `replayed_from_id`) — ran `pnpm db:migrate` → 050–052 applied; (2) the portal "Setting up developer session…" hang + reference listener browser-barrel import — ship-90's PR #307, merged and ff'd into the shared checkout so Vite/HMR picked it up. Audit rows at 12:13 local show Troy's own dry run succeeded (POST /webhooks → POST /documents → DELETE cleanup under the demo app).
