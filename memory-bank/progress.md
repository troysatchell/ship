...
| **TRO-609 — e2e serial-mode cascade (sprint filter, never-built bucket filters)** | 🟡 **PR #266 open, In Review (2026-08-16 ~05:24Z)** — 54→46 tests, 8 never-built deleted, 4 fixed, 1 regression test added, root cause traced to `ed932c9`. Gate has 2 disclosed exceptions (justified net test-loss + TRO-277-class load flakes), not an unqualified pass. TRO-613 filed for a separate unfixed failure found deeper in the same file. |
