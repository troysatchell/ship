...
**Tech Stack:** TypeScript (`typescript` compiler API for AST parsing), `tsx` (already a workspace devDependency) for execution, `vitest` (root devDependency) for tests, bash (`gate.sh`).
