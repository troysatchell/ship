 ...
 * early ("SKIP: No nested documents"). It could not fail. It was one of the
 * three specs the brace-scanner found with no assertion at all, and it sits next
 * to the sidebar ARIA work (A11Y-1 / TRO-215) it appears to have been written to
 * investigate.
 *
 ...
 * *contract* and the keyboard-reachability of the control. It says nothing about
 * what a screen reader announces. That still requires a human.
 */
...
test.describe('Document sidebar disclosure ARIA', () => {
  test('check aria-expanded elements', async ({ page }) => {
    await login(page);
    await page.goto('/docs');

    const sidebar = page.locator('#sidebar-content, aside[aria-label="Document list"]').first();
    await expect(sidebar, 'the document list sidebar should render on /docs').toBeVisible({
      timeout: 15000,
    });

    // Seed data guarantees a nested document, so an expandable row must exist.
    // The old version `return`ed here when it found none, which is exactly why
    // it never reported the absence.
    ...
    // A11Y-1: aria-expanded belongs on the interactive control. An <li> or a
    // <div> carrying it is precisely the defect that finding described.
    const tagNames = await expanders.evaluateAll((els) => els.map((el) => el.tagName));
    expect(tagNames.length, 'at least one aria-expanded element should be present').toBeGreaterThan(
      0
    );
  ...
  test('aria-expanded tracks the visible children and survives navigating into one', async ({
    ...
    // "Welcome to Ship" is the seeded parent; target it by name so the test
    // cannot silently pick up an unrelated row.
    ...
    // Collapse first if it happens to start open, so the assertion below is
    // about the toggle rather than about the initial state.
    ...
    // Navigating into the child must leave the parent expanded — a disclosure
    // that silently collapses loses the user's place.
    await child.click();
