...
function buildTimeline(actionItems: ActionItem[], weekNumber: number): TimelineDay[] {
  ...
  // Fixed-length above, but indexed access still needs a concrete reference
  // for the type checker (and to fail loudly if the shape above ever shrinks).
  const [monday, , , thursday] = days;
  if (!monday || !thursday) {
    throw new Error('buildTimeline: expected 7 fixed weekday entries');
  }
