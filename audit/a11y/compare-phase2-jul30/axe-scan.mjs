// COMPARE-MODE copy of audit/a11y/axe-scan.mjs — identical methodology (same
// axe-core tags, same viewport, same states, same modal-dismiss handling),
// only the BASE port and output dir changed to this worktree's allocation
// (WEB_PORT from .factory-env) so it runs against this worktree's server.
...
if (!WEB_PORT) { console.error('Set WEB_PORT (from .factory-env) before running.'); process.exit(1); }
...
const WIKI = process.env.WIKI_DOC_ID || '';
if (!SESSION) { console.error('Set SESSION_ID (fresh session_id cookie) before running.'); process.exit(1); }
if (!WIKI) { console.error('Set WIKI_DOC_ID (a seeded wiki doc id) before running.'); process.exit(1); }
const OUT = 'audit/a11y/compare-phase2-jul30/axe';
...
// The seeded dev user MAY have an overdue action item, so an "Action Items"
// role=dialog can auto-open on every navigation and trap focus. Dismiss it so
// page scans/keyboard probes measure the actual page, not the modal. Record
// whether Escape works and how many times it was encountered (0 is valid —
// it means no accountability item was pending on this measurement date).
...
try {
  ...
  console.log('\nWROTE audit/a11y/compare-phase2-jul30/axe/_summary.json');
} finally {
  await browser.close();
}
