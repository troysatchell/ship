...
**VERIFIED 59 · PARTIAL 9 · ASSUMED 3 · IMPLEMENTED-UNVERIFIED 3 · MISSING 2 · N/A 3** (79 active requirements).
...
- **Three rows carry `ASSUMED` below the flood cap** (W6-R31, R36, R55) — deliberately: the sweep ran unattended, each is a genuine two-reading ambiguity with no governing interpretation, and the yes/no questions are put to Troy at the end of this run (see *Blocked / assumed*). Each will re-tier to a settled verdict once ruled and the ruling is logged in `interpretations.md`.
...
14 rows below the VERIFIED bar that are not N/A (2 MISSING, 9 PARTIAL, 3 ASSUMED) — full handoff in `gaps-W6-2026-08-16b.md`. In plain English, grouped by who can close them:
...
- **W6-R36 ASSUMED→likely PARTIAL** — an *Audit* page under `/developer/*` reading `GET /api/v1/audit` (endpoint + SDK method exist; portal token already carries `audit:read`).
...
**Interpretation-dependent (asked of Troy at the end of this run):**
- **W6-R31 ASSUMED** — VERIFIED if a localStorage `ITokenStore` living in `integrations/browser-demo` (not exported by `@ship/sdk`) counts.
- **W6-R55 ASSUMED** — VERIFIED if "containerized Ship instance" accepts Postgres-in-container + the real API spawned from the checkout; PARTIAL if the API itself must run from an image.
...
No `BLOCKED` rows. Three `ASSUMED` rows, each with a yes/no question pending Troy's ruling (to be logged as I-05..I-07 in `interpretations.md`):

| ID | Question (yes/no) | Traced under | If YES | If NO |
|---|---|---|---|---|
| W6-R31 | Does a browser-localStorage `ITokenStore` implemented in `integrations/browser-demo` (an SDK consumer) — not exported by `@ship/sdk` — satisfy "Pluggable ITokenStore (in-memory, file, browser localStorage)"? | yes | VERIFIED (all flows/stores green this sweep) | PARTIAL — export a `LocalStorageTokenStore` from the SDK's browser-safe barrel |
| W6-R36 | Is the admin-scoped `GET /api/v1/audit` + SDK `audit.list()` enough for "Queryable in the developer portal", with no portal UI page? | no (UI required) | VERIFIED (API + SDK green) | PARTIAL — add the portal Audit page |
| W6-R55 | Does "against a containerized Ship instance" accept Postgres-in-a-container + the real Ship API spawned from the checkout (the drill's current shape)? | no (strict) | VERIFIED (30 green CI runs) | PARTIAL — opt-in image-backed API path in the drill |
| W6-R52 (verdict not contingent) | Does a per-run 2000 ms ceiling on the drill's `wait_for_delivery` stage satisfy "P95 < 2 s", or must an aggregated P95 over ≥20 runs be asserted? | per-run ceiling acceptable | scope = lower the stage ceiling | scope = aggregate CI runs and assert P95 |

## Delta (compare mode, vs. 2026-08-16 07:10Z sweep)

Only rows whose verdict changed (21). Net: 12 up, 9 down — the downgrades are corrections to prior over-crediting or newly-observed missing halves, not regressions on `main` (no W6 code path that was green is now red).
...
**GO if the ruling lands PARTIAL (ask Troy first — see Blocked/assumed):**
