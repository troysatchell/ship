...
**Commit:** 08505d2d459ac00842110d6410c2aef82c349e06 (== `origin/main` at sweep start; working tree dirty only in `memory-bank/activeContext.md`, `.claude/skills/ship-factory/SKILL.md`, `audit/factory/scorecard.jsonl` — none cited) · **Date:** 2026-08-16T13:35Z–14:20Z · **Docs:** W6 (`GFA_Week_6_PlugForge.pdf`, 18 pp, sha256 `81a3788d…` unchanged → inventory reused, 79 active requirements) · **Mode:** compare (`w6-2026-08-16b`, against `matrix.after-w6-2026-08-16.json`; `baselineRef` = `matrix.baseline-W6.json`)

Companion files: `matrix.after-w6-2026-08-16b.json` (machine-readable, every field), `gaps-W6-2026-08-16b.md` (PM handoff), `sweep-w6-2026-08-16b/` (captured logs: `test.log`, `test-*.log`, `typecheck.log`, `e2e-targeted.log`, `d-*.txt` schema dumps, `citation-check.txt`, per-cluster tracer JSON).

## Summary

**VERIFIED 59 · PARTIAL 12 · IMPLEMENTED-UNVERIFIED 3 · MISSING 2 · N/A 3** (79 active requirements). *(At the end of the run three rows were `ASSUMED` pending four yes/no rulings; Troy ruled all four at ~14:35Z — I-05..I-08 in `interpretations.md` — and the three re-tiered to PARTIAL. Counts above are post-ruling.)*

In plain terms: **the platform is built and behaviorally green — 59 of 79 brief requirements are proven by a test or probe run *this sweep* (unit suites 2,905/2,906 across api/web/agent/sdk/cli on a throwaway database, seven targeted Playwright specs, live-deploy GETs, and CI drill history), and only 2 requirements have nothing at all behind them (both are human-only submission artifacts: the pre-search doc and the social post).** The most consequential finding is a **correction, not a regression**: the prior sweep credited two graded testing scenarios that the drill does not actually assert — **W6-R46** (the "signed POST within 2 s + tamper-rejection" scenario: `scripts/drill/ttfe.ts` has no tamper step, and its only latency bound is a 15 000 ms stage budget) and **W6-R52** (webhook first-attempt P95 < 2 s: the "2000" in `ttfe.config.json` belongs to the *verify_webhook* stage, not delivery latency). Observed CI latencies (599–690 ms) satisfy the number, but nothing asserts it; both rows are re-tiered to PARTIAL with a ~10-line fix each. A second correction: **W6-R36**'s "queryable in the developer portal" — the portal has **no audit-trail view** (`grep '/api/v1/audit' web/src` → nothing; `DeveloperSidebar` nav is Apps + Webhooks only); the prior sweep inferred it from the portal existing. Beyond those, what remains open is exactly what needs Troy: the destroy-redeploy/drift proof on the real Render topology (TRO-415, ⚠️ human go-ahead), the pre-search + demo video + social post (TRO-429/TRO-444), a token-volume measurement for the E7 rewire (no ticket), and the E7 section of the epic write-ups.

## Coverage and limitations

- **What ran (all against a throwaway DB `ship_req_audit_w6b_scratch`, created, migrated 001..051, dropped after — no shared data touched):** `pnpm type-check` (exit 0, 8 packages); `pnpm test` (api suite 1468/1469 — see next bullet — the root `&&` chain then stopped, so `test:web` 606/606, `test:agent` 544/544, `test:sdk` 229/229, `test:cli` 58/58 were run individually); 7 targeted Playwright specs (`PLAYWRIGHT_WORKERS=2`, per-worker Postgres testcontainers) — 12 passed, **2 flaky-then-passed** (named under *Verification performed*).
- **One unit-test failure, disclosed:** `api/src/__tests__/activity.test.ts › only queries entities in the authenticated workspace` failed in the full run (expected 200, got 400; mocked-query test of an *internal* route, no W6 row bears on it) and **passed 2/2 when re-run in isolation** — consistent with the repo's known load-sensitive flake class (TEST-12/TRO-277; six tracer subagents were saturating the machine). It is recorded as a flake, not dismissed: it is the reason `pnpm test`'s root exit code was 1.
- **Not run:** the full Playwright regression suite (70+ specs — W6-R10 rests on the ticket trail and the 08-14 numeric compares); `terraform plan/validate` (no binary here — W6-R12/R38/R40 rest on committed plan artifacts and PR #273's recorded `terraform validate`); the drill itself locally (`scripts/drill/` is not a workspace package; W6-R49/R50/R56/R57/R59 rest on **GitHub CI drill-ttfe jobs observed read-only via `gh` on the traced commit** — 30 consecutive successes, run IDs in the matrix notes; GitLab side is config-only evidence).
- **Statically traced only (IMPLEMENTED-UNVERIFIED, 3 rows):** W6-R38 (terraform topology; mechanical artifact check passed, no plan re-run), W6-R39 (IAM memo — the defended Render adaptation exists; no AWS before/after policy artifact can exist), W6-R76 (three discovery docs read, content not graded).
- **Ticket dimension:** OK — 113 issues in the PlugForge Linear project pulled 13:45Z; not the whole TRO team (six interleaved projects). 17 orphans, of which 10 are EPIC parents.
- **Three rows were traced `ASSUMED` below the flood cap during the run** (W6-R31, R36, R55 — genuine two-reading ambiguities, no governing interpretation, sweep unattended) and **were ruled at the end of the run** (I-05..I-08). All three settled to PARTIAL under Troy's "take the requirements at face value" instruction; W6-R31's ruling was applied by the audit session under that instruction (Troy did not rule the line individually) and is marked as such in I-06 — overridable.
- **Line drift:** the tracers re-opened all 400+ citations against `08505d2d`; every one resolves (`sweep-w6-2026-08-16b/citation-check.txt`, 0 bad). Several prior citations moved (e.g. `documents.ts` +4 lines after PR #276; `events.ts` 99→110/237→248) and were re-pointed.
- **Main moved during the sweep:** GitHub main gained a newer head (`a3ef8ee7`, in-progress CI at 13:41Z) after tracing began; this report is about `08505d2d`.

## Matrix

One row per active requirement (full evidence lists, notes and plain-English text in the JSON and in the section after this table).
...
| W6-R31 | ShipClient.authorizationCodeFlow() and ShipClient.deviceLogin() handle their flows end-to-… | TRO-418, TRO-600, TRO-425, TRO-449 | `sdk/src/client.ts:186`<br>`sdk/src/client.ts:222`<br>`sdk/src/deviceLogin.ts:188`<br>`sdk/src/deviceLogin.ts:219`<br>+9 more | **PARTIAL** |
...
| W6-R55 | CLI drill harness: pnpm drill ttfe runs the full loop end-to-end against a containerized S… | TRO-455 | `package.json:36`<br>`scripts/drill/run.ts:13`<br>`scripts/drill/ttfe.ts:349`<br>`scripts/drill/ttfe.ts:386`<br>+4 more | **PARTIAL** |
...
## Requirement-by-requirement, in plain English

Each entry: what the brief asks for → what exists in the repo (file:line) → which ticket delivered it → what, if anything, is still missing.
...
14 rows below the VERIFIED bar that are not N/A (2 MISSING, 12 PARTIAL after the rulings) — full handoff in `gaps-W6-2026-08-16b.md`. In plain English, grouped by who can close them:

**Needs Troy personally (human checkpoints / recordings):**
- **W6-R73 MISSING** — Pre-Search doc with Troy's answers + saved AI conversation (TRO-429 🔔 Backlog). Nothing W6-specific exists; `PRESEARCH.MD` is Week 5's.
- **W6-R78 MISSING** — Social post tagging @GauntletAI with the `ship webhooks tail` verified-event screenshot (TRO-444 Backlog). All the tooling to produce the screenshot is merged (TRO-452).
- **W6-R72 PARTIAL** — Demo video following the five-line story + portal replay (TRO-444/429). A script exists (`docs/submission/PLUGFORGE-DEMO-SCRIPT.md`) but it is a stale curl-based rehearsal, and there is no recording.
- **W6-R12 / W6-R40 PARTIAL** — Destroy-and-redeploy + drift demo on the *real* `ship`/`ship-db` Render resources (TRO-415 ⚠️ HUMAN GO-AHEAD, Backlog). Existing proofs are for the W5 agent service and a local-provider fixture.

**Small engineering fixes (each ~a ticket, none has one yet unless named):**
- **W6-R46 PARTIAL** — add a tamper-negative step and an explicit ≤2 s delivery assertion to `scripts/drill/ttfe.ts`.
- **W6-R52 PARTIAL** — assert the P95<2 s first-delivery target (currently only recorded: 690 ms observed in CI).
- **W6-R36 PARTIAL (ruled I-05)** — an *Audit* page under `/developer/*` reading `GET /api/v1/audit` (endpoint + SDK method exist; portal token already carries `audit:read`).
- **W6-R64 PARTIAL** — measure token volume in both `AGENT_PLATFORM_MODE`s and fill PF-905 §2.1's TODO; `agent/src/shipClient.ts:548-563` still nulls `content` in sdk mode after TRO-605 widened the server.
- **W6-R75 PARTIAL** — epic write-ups lack the E7 section (audit rows) and cite a local drill run instead of a CI run ID for E6.
- **W6-R71 PARTIAL** — only 1 of 4 sampled PR bodies names the acceptance criterion + "fitness test passed"; a `PULL_REQUEST_TEMPLATE.md` would fix it going forward.
- **W6-R10 PARTIAL** — TRO-613 (In Progress) keeps `program-mode-week-ux.spec.ts` red on main; the +10% numeric compares are from 08-14 and should be re-run on current main.

**Ruled at the end of the run (face-value readings, I-06/I-07/I-08):**
- **W6-R31 PARTIAL** — `@ship/sdk` must itself export a browser-localStorage `ITokenStore` (today it lives only in `integrations/browser-demo`); ~20 min.
- **W6-R55 PARTIAL** — the drill must run the Ship API from a container image, not `tsx` from the checkout; opt-in image-backed path, 2–3 h, GitHub CI only.
- **W6-R52 PARTIAL** — the fix must compute a real P95 over ≥20 drill runs, not just tighten a per-run stage ceiling.

**Observations that are not gaps against a brief line but should become tickets:** (a) `sdk/src/types.ts:170` `IssuePriority` lacks `'none'` after PR #276 widened shared/OpenAPI — the SDK↔spec parity test passes because it walks operations, not enum members (W6-R45 stays VERIFIED on its stated bar); (b) `integrations/slack`'s 6-test vitest suite is wired into no runner or CI job (type-check only); (c) the two flaky portal e2e assertions (below) violate `e2e/AGENTS.md`'s own no-point-in-time-checks rule; (d) `README.md:387` still says the portal "doesn't exist yet".

## Orphan tickets

17 tickets in the PlugForge project map to no inventory requirement (list in `gaps-W6-2026-08-16b.md`): 10 EPIC parents (TRO-386..395, organizational), TRO-400 (PF-201 issues/sprints/me — PRD scope beyond the brief's literal lines, real and Done), TRO-490 (pre-existing YAML converter bug), TRO-587 (CodeQL false-positive dismissal), TRO-610 (session-timeout button, unrelated), TRO-612 (test-guard hardening), TRO-606/608 (duplicates of TRO-604). None indicates work done against the wrong project.

## Blocked / assumed

No `BLOCKED` rows. Three rows were `ASSUMED` during the run and are now ruled (verdicts in the matrix are post-ruling):

| ID | Question (yes/no) | Ruling (2026-08-16, Troy) | Result |
|---|---|---|---|
| W6-R36 | Is `GET /api/v1/audit` + SDK `audit.list()` enough for "Queryable in the developer portal", with no portal UI page? | **No — portal UI page required** (I-05) | PARTIAL; PM GO item 5 (Audit page) |
| W6-R31 | Does a browser-localStorage `ITokenStore` living in `integrations/browser-demo`, not exported by `@ship/sdk`, satisfy "Pluggable ITokenStore (in-memory, file, browser localStorage)"? | Troy: "I don't know what this means — can't you tell yourself?" + batch instruction "take the project requirements at face value" → applied as **No — the SDK must ship it** (I-06, applied-by-audit, overridable) | PARTIAL; ~20-min export ticket |
| W6-R55 | Does Postgres-in-a-container + the real Ship API spawned from the checkout satisfy "against a containerized Ship instance"? | **No — the Ship API must run from an image** (I-07) | PARTIAL; 2–3 h opt-in image-backed drill path |
| W6-R52 | Does a per-run 2000 ms ceiling on the drill's `wait_for_delivery` stage satisfy "P95 < 2 s", or must an aggregated P95 over ≥20 runs be asserted? | **Face value → aggregated P95 must be computed and asserted** (I-08) | PARTIAL (unchanged); scope = gh/artifact-fed P95 over ≥20 drill runs |

## Delta (compare mode, vs. 2026-08-16 07:10Z sweep)

Only rows whose verdict changed (20 after rulings). Net: 12 up, 8 down — the downgrades are corrections to prior over-crediting or newly-observed missing halves, not regressions on `main` (no W6 code path that was green is now red).
...
| W6-R31 | VERIFIED | PARTIAL | ruled I-06: localStorage store must be exported by @ship/sdk; it lives only in integrations/browser-demo |
| W6-R36 | IMPLEMENTED-UNVERIFIED | PARTIAL | correction + ruled I-05: portal has no audit page (grep web/src for /api/v1/audit → nothing); API/SDK half verified |
...
| W6-R55 | PARTIAL | PARTIAL (ruled I-07, strict) | no verdict change; scope sharpened — the API must run from an image |
...
|---|---|---|
| `pnpm db:migrate` (DATABASE_URL → throwaway `ship_req_audit_w6b_scratch`; dropped after) | exit 0 — 56 migrations applied (001..051); `\d` dumps confirm `oauth_apps` (client_id, client_secret_hash, redirect_uris, requested_scopes, owner_user_id, is_first_party, revoked_at), `oauth_tokens` (family_id, parent_id, nullable user_id), `api_tokens.scopes text[]`, `webhook_deliveries` (attempt_number, response_status, response_excerpt, latency_ms, idempotency_key), `public_api_audit` (request_id, app_client_id, user_id, method, route, scope_used, status, latency_ms), `oauth_device_codes` (`user_code` still plaintext — TRO-589 open) | R13, R15, R18, R24, R28, R36 |
| `pnpm type-check` | exit 0 — 8/8 packages (`typecheck.log`) | R66, R9, R34 |
| `pnpm test` (root, scratch DB) | api: 124/125 files, **1468/1469 tests**; sole failure `activity.test.ts` (internal, non-W6) → root chain stopped | all api-backed rows (see matrix `commands_run`) |
| `npx vitest run src/__tests__/activity.test.ts` ×2 (api, scratch DB) | 13/13, 13/13 — flake, not a regression | — |
| `pnpm test:web` | 86 files / 606 tests passed | R27, R36, R37, R48 |
| `pnpm test:agent` (scratch DB) | 40 files / 544 tests passed (auditTrailProof.liveServer, shipClientParity.liveServer, gate/graph write-boundary) | R63, R67 |
| `pnpm test:sdk` (scratch DB) | 25 files / 229 tests passed (parity 57, verifyWebhook 30, all liveServer suites) | R9, R30–R34, R43, R45, R58, R60 |
| `pnpm test:cli` (scratch DB) | 9 files / 58 tests passed | R43, R62 |
| `PLAYWRIGHT_WORKERS=2 pnpm exec playwright test` × 7 specs (oauth-pkce-chain, oauth-authorize, browser-demo-pkce, developer-portal-apps, developer-portal-dlq-replay, oauth-refresh-rotation-stolen-token, webhook-idempotency-key-drill) | exit 0 — **12 passed, 2 flaky-then-passed**: `developer-portal-apps › portal calls hit the real /api/v1 surface` (first attempt: `v1Requests.some(/api/v1/me)` false — `waitForLoadState('networkidle')` raced the fetch) and `developer-portal-dlq-replay › subscription CRUD from the portal UI` (first attempt: `getByLabel(/^app$/i)` 15 s timeout). Scoped run — says nothing about the other specs. | R3, R14, R18, R24, R27–R29, R31, R37, R42, R48, R51, R61, R77 |
| `mcp__linear__list_issues` (project-scoped) | 113 issues: 82 Done · 19 Backlog · 8 In Progress · 2 In Review · 2 Duplicate | ticket_mapping |
| `gh run list/view` (drill-ttfe, last 40 CI runs) — tracer E | 31 success / 1 superseded-cancel / 0 fail; 30 consecutive green; run on traced commit total 3 990 ms, delivery stage 599–690 ms | R49, R50, R56, R57, R59 |
| `curl` GET `https://ship-rr6m.onrender.com/{health,api/v1/openapi.json,developer/apps}` — tracers B/F | 200 / valid OpenAPI 3.1.0 (21 paths, byte-identical to `docs/openapi.json`) / 200 with portal chunks in the bundle | R8, R11, R74, R77 |
| `pnpm openapi:check`, `node sdk/scripts/measure-size.mjs` (+ `--threshold-kb 1` → exit 1), `node scripts/check-integration-deps.mjs` (+ scratch violating pkg → exit 1), `bash scripts/factory/verify-terraform-artifact.sh …` — tracers | all OK / PASS as recorded per row | R21, R60, R69, R74, R38 |
| `pnpm test:e2e` (full suite) | **NOT RUN** — out of budget for an audit sweep | R10 |
| `terraform plan/validate` | **NOT RUN** — no binary here | R12, R38, R40 |

Captured excerpts for every VERIFIED row are in the matrix's per-row `verification.result_excerpt`; raw logs in `sweep-w6-2026-08-16b/`.

## PM handoff

Dispatched to `ship-pm` (config `pm_skill`) with the gaps file — response appended below when it returns.

### ship-pm scope-gate decision (2026-08-16 ~14:25Z, deadline context: Sunday 11:59 AM CT reading = ~16:59Z today, ≈2.5 h away)

The audit session is barred from creating Linear tickets (requirements-audit hard rule), so this is a **written decision for the factory/next session to file**, ranked by grade impact per hour. Nothing below reopens a Done ticket; each is a fresh, one-change ticket.

**GO — file and dispatch now (apply-tier, each ≤ 1 h, all grader-visible):**
1. **Drill asserts the two graded scenarios it currently only observes** (closes W6-R46 + W6-R52): in `scripts/drill/ttfe.ts` add a tamper-negative on the real captured delivery (`verifyWebhook(...) === false` after flipping a byte of `rawBody`) and an explicit `wait_for_delivery ≤ 2000 ms` assertion (a per-run ceiling is stronger than P95; if Troy rules "aggregate P95", add a `gh`-fed aggregation later — do not block on it). Proof: drill-ttfe CI job green + the new lines. *Why now:* p.5 graded-test rows are the rubric; observed 599–690 ms means the assertion will pass first try.
2. **Epic write-ups: add the E7 proof section + cite a CI run ID for E6** (closes W6-R75; docs-only): E7 = `auditTrailProof.liveServer.test.ts` rows + `PF-704-COST-LEDGER-DELTA.md`; E6 = run 31949732432. Fold in the stale `README.md:387` "portal doesn't exist yet" line. *Why now:* p.13 submission line, zero risk.
3. **`.github/PULL_REQUEST_TEMPLATE.md`** with "Acceptance criterion advanced:" / "Fitness test passed:" fields (advances W6-R71 going forward). 5 minutes.
4. **SDK `IssuePriority` gains `'none'`** (`sdk/src/types.ts:170`) + a parity assertion on enum members (observation (a)) — the SDK now rejects/mistypes a value the server emits since PR #276. Applier ticket.

**GO — ruled PARTIAL after the run (I-05/I-06/I-07), file as tickets; 5 and 6 are small, 7 is not a today item:**
5. **Portal Audit page** (W6-R36) — `DeveloperSidebar` nav entry + page calling `GET /api/v1/audit` via `usePortalToken().callV1`, cursor-paginated, `app_client_id` filter, RTL test. ~1–2 h; only worth starting today if the ruling is "UI required" **and** items 1–2 are already in flight. My recommendation on the ruling: the brief's words are "queryable in the developer portal" — I would read that as UI and file it.
6. **`LocalStorageTokenStore` exported from `@ship/sdk`** (W6-R31) — 20 min if ruled strict; recommend Troy rule *lenient* (the brief lists store kinds, not export locations; the browser demo proves the pluggability), which closes the row without code.
7. **Image-backed drill path** (W6-R55) — recommend Troy rule *lenient*: the drill installs the SDK into a clean directory and talks to a real Ship over HTTP with Postgres containerized; the API-from-image variant is GitHub-only and buys no grader-visible behaviour today. Ruling strict → 2–3 h, not before the deadline.

**NOT NOW — needed but not efficient before the deadline; file for after:**
8. **E7 token-volume measurement + `shipClient.ts` content passthrough** (W6-R64): requires live LLM runs in both `AGENT_PLATFORM_MODE`s to fill PF-905 §2.1; the doc honestly says "not yet measured", which is defensible in the interview. File it; do not start it in the deadline window.
9. **Slack `server.test.ts` wired into `pnpm test` + CI** (observation (b)) and **the two portal e2e point-in-time assertions** (observation (c), `developer-portal-apps.spec.ts:119`, `developer-portal-dlq-replay.spec.ts` label wait) — real hygiene, no rubric line; after.
10. **W6-R10 re-run of the three +10% compares on current main** — audit chore once TRO-613 lands; after.

**HUMAN — not ticketable to an agent, escalate to Troy now:** TRO-429 pre-search + saved conversation (W6-R73), TRO-444 demo video + social post (W6-R72/R78 — everything the video needs is merged), TRO-415 destroy-redeploy + drift on `ship`/`ship-db` (W6-R12/R40 — ⚠️ destroys the live deploy; only with an explicit go-ahead and only if the video is already recorded, since it takes the site down).

**Dismissed / no action:** the 17 orphans (10 epic parents; TRO-400 is PRD-scoped work the brief's literal lines don't name; the rest are hardening/dup/unrelated — none is misfiled); the `activity.test.ts` single failure (isolated 2/2 pass, TEST-12 class already tracked under TRO-277 — no new ticket).
