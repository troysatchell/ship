// db-query-audit COMPARE (phase2-jul30): parse the Postgres statement log into per-flow
// query slices. Byte-identical logic to baseline's audit/db-query/raw/parse-log.mjs;
// only the two output paths below were changed to keep compare artifacts separate.
// Run: node audit/db-query/compare-phase2-jul30/raw/parse-log-compare.mjs audit/db-query/compare-phase2-jul30/raw/pg-statements.log
...
writeFileSync('audit/db-query/compare-phase2-jul30/raw/flow-queries.json', JSON.stringify(out, null, 2));
...
writeFileSync('audit/db-query/compare-phase2-jul30/raw/top-statements.json', JSON.stringify(top, null, 2));
